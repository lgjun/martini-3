...
## OPTIONS ##
  ...
  INITRAM options:
    ...
    -ntmpi   Number of processors/threads                                 *INT:   $NTMPI      #添加
    -ntomp                                                                *INT:   $NTOMP      #添加
    -gpu_id                                                               *INT:   $GPU_ID     #添加
...
while [ -n "$1" ]; do
  case $1 in
    ...
    -ntmpi)                 NTMPI=$2      ; shift 2; continue ;;      #添加
    -ntomp)                 NTOMP=$2      ; shift 2; continue ;;      #添加
   -gpu_id)                GPU_ID=$2      ; shift 2; continue ;;      #添加
...
energygrps=Protein 
;energygrp_excl=Protein Protein   #分号注释掉，避免报错：Energy group exclusions are currently not supported

; for gmx5, need cutoff-scheme = Group to allow energy group exclusions
cutoff-scheme = Verlet    # 改成Verlet,避免报错：The group cutoff scheme has been removed since GROMACS 2020. Please use the Verlet cutoff scheme.
coulombtype   = PME     # 添加后避免Note: You are using a plain Coulomb cut-off, which might produce artifacts. You might want to consider using PME electrostatics.
...
# Run
M="gmx mdrun -deffnm $BASE -v -ntmpi $NTMPI -ntomp $NTOMP -gpu_id $GPU_ID"    #更改
echo $M; $M 
...
do
  ...
  echo $G; $G || exit
  M="gmx mdrun -deffnm $tag -v -ntmpi $NTMPI -ntomp $NTOMP -gpu_id $GPU_ID"   #更改
  echo $M; $M 
  GRO=$tag.gro
